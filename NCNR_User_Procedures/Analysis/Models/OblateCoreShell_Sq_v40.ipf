...
//two yukawa
Proc PlotOblate_2Y(num,qmin,qmax)
	Variable num=128,qmin=0.001,qmax=0.7
	Prompt num "Enter number of data points for model: "
	Prompt qmin "Enter minimum q-value (A^-1) for model: "
	Prompt qmax "Enter maximum q-value (A^-1) for model: "
	
	Make/O/D/n=(num) xwave_OEF_2Y,ywave_OEF_2Y
	xwave_OEF_2Y =  alog(log(qmin) + x*((log(qmax)-log(qmin))/num))
	Make/O/D coef_OEF_2Y = {0.01,100,50,110,60,1e-6,2e-6,6.3e-6,6,10,-1,2,0.0001}
	make/o/t parameters_OEF_2Y = {"volume fraction","major core radius (A)","minor core radius (A)","major shell radius (A)","minor shell radius (A)","SLD core (A-2)","SLD shell (A-2)","SLD solvent (A^-2)","scale, K1","charge, Z1","scale, K2","charge, Z2","bkg (cm-1)"}
	Edit parameters_OEF_2Y,coef_OEF_2Y
	
	Variable/G root:g_OEF_2Y
	g_OEF_2Y := Oblate_2Y(coef_OEF_2Y,ywave_OEF_2Y,xwave_OEF_2Y)
	Display ywave_OEF_2Y vs xwave_OEF_2Y
	ModifyGraph log=1,marker=29,msize=2,mode=4
	Label bottom "q (A\\S-1\\M)"
	Label left "Intensity (cm\\S-1\\M)"
	...
	AddModelToStrings("Oblate_2Y","coef_OEF_2Y","parameters_OEF_2Y","OEF_2Y")
End

// - sets up a dependency to a wrapper, not the actual SmearedModelFunction
Proc PlotSmearedOblate_2Y(str)								
	String str
	Prompt str,"Pick the data folder containing the resolution you want",popup,getAList(4)
	...
	// Setup parameter table for model function
	Make/O/D smear_coef_OEF_2Y = {0.01,100,50,110,60,1e-6,2e-6,6.3e-6,6,10,-1,2,0.0001}
	make/o/t smear_parameters_OEF_2Y = {"volume fraction","major core radius (A)","minor core radius (A)","major shell radius (A)","minor shell radius (A)","SLD core (A-2)","SLD shell (A-2)","SLD solvent (A^-2)","scale, K1","charge, Z1","scale, K2","charge, Z2","bkg (cm-1)"}
	Edit smear_parameters_OEF_2Y,smear_coef_OEF_2Y					
	
	// output smeared intensity wave, dimensions are identical to experimental QSIG values
	// make extra copy of experimental q-values for easy plotting
	Duplicate/O $(str+"_q") smeared_OEF_2Y,smeared_qvals				
	SetScale d,0,0,"1/cm",smeared_OEF_2Y							
					
	Variable/G gs_OEF_2Y=0
	gs_OEF_2Y := fSmearedOblate_2Y(smear_coef_OEF_2Y,smeared_OEF_2Y,smeared_qvals)	//this wrapper fills the STRUCT
	
	Display smeared_OEF_2Y vs smeared_qvals									
	ModifyGraph log=1,marker=29,msize=2,mode=4
	Label bottom "q (A\\S-1\\M)"
	Label left "Intensity (cm\\S-1\\M)"
	...
	SetDataFolder root:
	AddModelToStrings("SmearedOblate_2Y","smear_coef_OEF_2Y","smear_parameters_OEF_2Y","OEF_2Y")
End
	

Function Oblate_2Y(w,yw,xw) : FitFunc
	Wave w,yw,xw
	...
	//setup structure factor coefficient wave
	Make/O/D/N=6 struct_OEF_2Y
	struct_OEF_2Y[0] = w[0]
	struct_OEF_2Y[1] = 0.5*DiamEllip(Ras,Rbs)
	struct_OEF_2Y[2] = w[8]
	struct_OEF_2Y[3] = w[9]
	struct_OEF_2Y[4] = w[10]
	struct_OEF_2Y[5] = w[11]
	
	//calculate each and combine
	Duplicate/O xw temp_OEF_2Y_PQ,temp_OEF_2Y_SQ		//make waves for the AAO
	OblateForm(form_OEF_2Y,temp_OEF_2Y_PQ,xw)
	TwoYukawa(struct_OEF_2Y,temp_OEF_2Y_SQ,xw)
	yw = temp_OEF_2Y_PQ * temp_OEF_2Y_SQ
	yw *= w[0]
	yw += w[12]
	
	//cleanup waves
//	Killwaves/Z form_OEF_2Y,struct_OEF_2Y
...
// this is all there is to the smeared calculation!
Function SmearedOblate_2Y(s) :FitFunc
	Struct ResSmearAAOStruct &s

//	the name of your unsmeared model is the first argument
	Smear_Model_20(Oblate_2Y,s.coefW,s.xW,s.yW,s.resW)
...
// used only for the dependency, not for fitting
//
Function fSmearedOblate_2Y(coefW,yW,xW)
	Wave coefW,yW,xW
