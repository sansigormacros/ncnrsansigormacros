...
//two yukawa
Proc PlotPolyRectSphere_2Y(num,qmin,qmax)
	Variable num=256,qmin=0.001,qmax=0.7
	Prompt num "Enter number of data points for model: "
	Prompt qmin "Enter minimum q-value (A^-1) for model: "
	Prompt qmax "Enter maximum q-value (A^-1) for model: "
	
	Make/O/D/n=(num) xwave_RECT_2Y,ywave_RECT_2Y
	xwave_RECT_2Y =  alog(log(qmin) + x*((log(qmax)-log(qmin))/num))
	Make/O/D coef_RECT_2Y = {0.1,60,0.1,1e-6,6.3e-6,6,10,-1,2,0.0001}
	make/o/t parameters_RECT_2Y = {"volume fraction","avg radius (A)","polydispersity","SLD sphere (A^-2)","SLD solvent (A^-2)","scale, K1","charge, Z1","scale, K2","charge, Z2","bkg (cm-1)"}
	Edit/K=1 parameters_RECT_2Y,coef_RECT_2Y
	
	Variable/G root:g_RECT_2Y
	g_RECT_2Y := PolyRectSphere_2Y(coef_RECT_2Y,ywave_RECT_2Y,xwave_RECT_2Y)
	Display/K=1 ywave_RECT_2Y vs xwave_RECT_2Y
	ModifyGraph log=1,marker=29,msize=2,mode=4
	Label bottom "q (A\\S-1\\M)"
	Label left "Intensity (cm\\S-1\\M)"
	...
	AddModelToStrings("PolyRectSphere_2Y","coef_RECT_2Y","parameters_RECT_2Y","RECT_2Y")
End

// - sets up a dependency to a wrapper, not the actual SmearedModelFunction
Proc PlotSmearedPolyRectSphere_2Y(str)								
	String str
	Prompt str,"Pick the data folder containing the resolution you want",popup,getAList(4)
	...
	// Setup parameter table for model function
	Make/O/D smear_coef_RECT_2Y = {0.1,60,0.1,1e-6,6.3e-6,6,10,-1,2,0.0001}
	make/o/t smear_parameters_RECT_2Y = {"volume fraction","avg radius (A)","polydispersity","SLD sphere (A^-2)","SLD solvent (A^-2)","scale, K1","charge, Z1","scale, K2","charge, Z2","bkg (cm-1)"}
	Edit smear_parameters_RECT_2Y,smear_coef_RECT_2Y					
	
	// output smeared intensity wave, dimensions are identical to experimental QSIG values
	// make extra copy of experimental q-values for easy plotting
	Duplicate/O $(str+"_q") smeared_RECT_2Y,smeared_qvals				
	SetScale d,0,0,"1/cm",smeared_RECT_2Y							
					
	Variable/G gs_RECT_2Y=0
	gs_RECT_2Y := fSmearedPolyRectSphere_2Y(smear_coef_RECT_2Y,smeared_RECT_2Y,smeared_qvals)	//this wrapper fills the STRUCT
	
	Display smeared_RECT_2Y vs smeared_qvals									
	ModifyGraph log=1,marker=29,msize=2,mode=4
	Label bottom "q (A\\S-1\\M)"
	Label left "Intensity (cm\\S-1\\M)"
	...
	SetDataFolder root:
	AddModelToStrings("SmearedPolyRectSphere_2Y","smear_coef_RECT_2Y","smear_parameters_RECT_2Y","RECT_2Y")
End
	

Function PolyRectSphere_2Y(w,yw,xw) : FitFunc
	Wave w,yw,xw
	
	Variable inten
	
	//setup form factor coefficient wave
	Make/O/D/N=6 form_RECT_2Y
	form_RECT_2Y[0] = 1
	form_RECT_2Y[1] = w[1]
	form_RECT_2Y[2] = w[2]
	form_RECT_2Y[3] = w[3]
	...
	//setup structure factor coefficient wave
	Make/O/D/N=6 struct_RECT_2Y
	struct_RECT_2Y[0] = w[0]
	struct_RECT_2Y[1] = diam/2
	struct_RECT_2Y[2] = w[5]
	struct_RECT_2Y[3] = w[6]
	struct_RECT_2Y[4] = w[7]
	struct_RECT_2Y[5] = w[8]
	
	//calculate each and combine
	Duplicate/O xw temp_RECT_2Y_PQ,temp_RECT_2Y_SQ		//make waves for the AAO
	PolyRectSpheres(form_RECT_2Y,temp_RECT_2Y_PQ,xw)
	TwoYukawa(struct_RECT_2Y,temp_RECT_2Y_SQ,xw)
	yw = temp_RECT_2Y_PQ * temp_RECT_2Y_SQ
	yw *= w[0]
	yw += w[9]
	
	//cleanup waves
//	Killwaves/Z form_RECT_2Y,struct_RECT_2Y
...
// this is all there is to the smeared calculation!
Function SmearedPolyRectSphere_2Y(s) :FitFunc
	Struct ResSmearAAOStruct &s

//	the name of your unsmeared model is the first argument
	Smear_Model_20(PolyRectSphere_2Y,s.coefW,s.xW,s.yW,s.resW)
...
// used only for the dependency, not for fitting
//
Function fSmearedPolyRectSphere_2Y(coefW,yW,xW)
	Wave coefW,yW,xW
	...
	Variable err
	err = SmearedPolyRectSphere_2Y(fs)
