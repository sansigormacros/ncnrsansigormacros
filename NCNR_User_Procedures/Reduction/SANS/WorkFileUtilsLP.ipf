...
//	dt_ill=3.0e-6			//Cerca detector deadtime constant as of 30-AUG-99
	...
	NVAR doEfficiency =root:myGlobals:gDoDetectorEffCorrILL
	...
	// sum  up the new data array = total_det
...
//	total_det += dscale*realsread[2]		//summed up after scaling
...
//	data *= dscale		//deadtime correction done tube-by-tube in loop above
...
//			solidAngle[ii][jj] = ratio		//testing only	
			xy = xx[ii]*yy[jj]
			data[ii][jj] *= xy*ratio   // that should be ON Lionel
			...
			if(doEfficiency)
				data[ii][jj]  /= DetEffCorrILL(lambda,dtdist,xd)  
