...
Proc ShowDataFolderTree(fileName, level, sNBName)
	//	String dataFolderStr="root:"

	string   fileName      = "sans"
	variable level         = 0
	string   sNBName       = "DataFolderTree"
	string   DataFolderStr = ""

	datafolderStr = "root:Packages:NIST:RawSANS:" + filename
	ListDataFolder($dataFolderStr, level, sNBName)
...
Function ListDataFolder(DFREF dfr, variable level, string sNBName)
	...
	for(i = 0; i < numWaves; i += 1)
		...
		if(WaveType(dfr:$name, 1) == 1)
			...
			if(numpnts(w) > 1) //the Igor_Folder_Attributes wave has zero points, catch the error
				sPrintf sString, "%s%s (%d-D wave N=(%s)) val=%g\ttyp=%s\r", indentStr, name, WaveDims(w), PrintDims(w), w[0], PrintWaveType(w)
				...
				sPrintf sString, "%s%s \t%g\tVAR\r", indentStr, name, w[0]
			endif
		else
			WAVE/T wt = dfr:$name // text wave is assumed...
			//			sPrintf sString, "%s%s (wave) \"%s\"\ttyp=text\r", indentStr, name, wt[0]
			sPrintf sString, "%s%s   \t\"%s\"\tSTRING\r", indentStr, name, wt[0]
		endif
...
Function WriteBrowserInfo(string sString, variable vType, string sNBName)
...
Function/S PrintWaveType(WAVE w)
...
Function/S PrintDims(WAVE w)

	string str = ""
	variable ii
	variable num = WaveDims(w)
