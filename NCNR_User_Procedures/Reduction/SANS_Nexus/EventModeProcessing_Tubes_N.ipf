...
Proc EventCorrectionPanel()
	...
	if(exists("rescaledTime") == 1)
		...
		// workaround to fix unresponsive buttons on panel. only an issue with Igor 9, not Igor 8 or 10
		ShowTools/A
...
//
//
Function EC_ColorizeTimeButtonProc(STRUCT WMButtonAction &ba) : ButtonControl
...
//
//
Function EC_SortTimeButtonProc(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function EC_DisplayButtonProc(STRUCT WMButtonAction &ba) : ButtonControl
...
Function EC_DoDifferential(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function EC_FindOutlierButton(STRUCT WMButtonAction &ba) : ButtonControl
...
Function AccumulateSlices(variable mode)
...
/////
// for testing of unresponsive ControlBar on graph
// (based on Event Correction Panel)
// -- sent query to support@wavemetrics.com on 4/6/26
//

Proc Test_UnresponsiveControlBar()

	Make/O/D/N=1000 rescaledTime=p

	Display/W=(35, 44, 761, 533)/K=2 rescaledTime
	DoWindow/C Test_UnresponsiveControlBar
	ModifyGraph mode=4
	ModifyGraph mode=0
	ModifyGraph marker=19
	ModifyGraph rgb=(0, 0, 0)
...
Function Test_DoneButtonProc(STRUCT WMButtonAction &ba) : ButtonControl
	...
	switch(ba.eventCode)
		case 2: // mouse up
			...
			DoWindow/K Test_UnresponsiveControlBar
			break
		case -1: // control being killed
			break
		default:
