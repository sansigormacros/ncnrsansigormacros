...
// SEP 2022 --
//
// This procedure file includes all of the necessary functions to set up and fill a Nexus data
// structure in local Igor folders, and then write out that file.
//
...
//
//		SASCALC -- fields are filled out with information from the SASCALC panel plus default values
//					for the selected instrument.
//
// -- procedures select the dimensions and detector constants based on either the ksDetType ("Tubes" or "Ordela")
...
//
//
// All of the VAX specific calls are still here, at the bottom of the file, but have all been commented
// out - they are kept strictly for historical purposes
//
//
...
// with SASCALC - also used with WorkFileMath
//
...
// fStr = "root:toExport"
//
Function SetupNexusStructure(fStr,style)
	String fStr,style

	Variable nx, ny
	if(cmpstr(style,"Tubes")==0)
		// tube values
		nx = 112
		ny = 128
...
//
Function FillFakeNexusStructure(fStr,style)
	String fStr,style
...
//
// values as needed for the different "style" (= "Tubes" or "Ordela") as needed
// be sure to redimension as needed, or the waves will keep the old dimensions
//
...
/////////
// BELOW -- old routines for writing out VAX-style files. Not used at all with Nexus
// data, kept only for historical purposes
//
