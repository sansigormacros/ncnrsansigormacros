...
//AJJ Jan 2010
// fileStr = full path:name to a valid file
// setNum = number of set (used for naming) = 0, 1, or 2 (ONLY), (2016) 3 is now valid for 4th data set
Function LoadDataForNSORT(string fileStr, variable setNum)

	variable err = 0

	string nm0, nm1, nm2
	//String firstFileName = S_fileName
	variable begPts, endPts, numCols
	variable pt = 0
...
Function WriteNSORTedFile(WAVE q3, WAVE i3, WAVE sig3, string firstFileName, string secondFileName, string thirdFileName, string fourthFileName, string normTo, variable norm12, variable norm23, variable norm34, [WAVE res])
...
Function WriteOLDNSORTedFile(WAVE q3, WAVE i3, WAVE sig3, string firstFileName, string secondFileName, string thirdFileName, string fourthFileName, string normTo, variable norm12, variable norm23, variable norm34, [WAVE res])

	variable refNum, numCols
	variable err    = 0
	variable dialog = 1
	string str2
	string fullPath  = ""
...
//
// num2 = largest point number of wave2 in overlap region
// 1 = first dataset, 2= second dataset
Function NCNR_GetScalingInOverlap(variable num2, WAVE wave1q, WAVE wave1i, WAVE wave2q, WAVE wave2i)
...
Function ShowNSORTHelp(string ctrlName) : ButtonControl
...
//be in use
//
Function NSORT_DoneButton(string ctrlName) : ButtonControl
...
//
Function Plot_0_Button(string ctrlName) : ButtonControl
...
//
Function NSORTPickPathButton(string ctrlName) : ButtonControl
...
//
Function SetBegOrEnd(string ctrlName, variable varNum, string varStr, string varName) : SetVariableControl
...
//
Function WriteNSORTFileButton(string ctrlName) : ButtonControl
...
//
Function SetScale_12(string ctrlName, variable varNum, string varStr, string varName) : SetVariableControl
...
//re-sets the global variable
//
Function SetScale_23(string ctrlName, variable varNum, string varStr, string varName) : SetVariableControl
...
//re-sets the global variable
//
Function SetScale_34(string ctrlName, variable varNum, string varStr, string varName) : SetVariableControl
...
//
Function CheckProc(string ctrlName, variable checked) : CheckBoxControl
...
Function LowQPopMenuProc(string ctrlName, variable popNum, string popStr) : PopupMenuControl
...
//when menu is popped, it gets a valid list to display and updates the control
//
Function MedQPopMenuProc(string ctrlName, variable popNum, string popStr) : PopupMenuControl
...
//are to be NSORTed
//
Function HighQPopMenuProc(string ctrlName, variable popNum, string popStr) : PopupMenuControl
...
//are to be NSORTed
//
Function HighestQPopMenuProc(string ctrlName, variable popNum, string popStr) : PopupMenuControl
...
//
//if auto == 1, do the scaling, if 0, use manual scale values
Function DoAutoScaleFromPanel(variable auto)

	NVAR normTo = root:myGlobals:NSORT:gNormToNum
	variable setNum, norm12, norm23, norm34
	variable err       = 0
	string   fileStr   = ""
	string   tempName  = ""
	string   name1     = ""
...
//
Function Set3NSORTFiles(variable low, variable med, variable hi, string pref)
...
// -- still assuming ABS
//
Function Set4NSORTFiles(variable low, variable med, variable hi, variable highest, string pref)
...
Function GetHeaderInfoToCombineWave(string fname, string sname)
	...
	// -- replace the suffix (that doesn't exist in the Nexus file) -- with the filename for now
...
//
//
Function DoCombineFiles(string ctrlName)
	...
	variable ii, lowFile, medFile, hiFile, highestFile
...
//
Function CombineTableHook(string infoStr)
...
Function CombinePanelDone(string ctrlName)

	DoWindow/K CombinePanel
	return (0)
End

Function ClearCombineTable(string ctrlName)
...
Function TableToCombineAndSave(variable clear)
...
//
Function DoCombineFiles_byName(WAVE/T lowW, WAVE/T medW, WAVE/T hiW, WAVE/T saveW)
