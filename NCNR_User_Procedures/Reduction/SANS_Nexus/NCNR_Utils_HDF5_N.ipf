...
Function/S N_getResolution(variable inQ, variable lambda, variable lambdaWidth, variable DDet, variable apOff, variable S1, variable S2, variable L1, variable L2, variable BS, variable del_r, variable usingLenses, variable &SigmaQ, variable &QBar, variable &fSubS)
...
//
//these are the output quantities at the input Q value
Function/S N_get2DResolution(variable inQ, variable phi, variable lambda, variable lambdaWidth, variable DDet, variable apOff, variable S1, variable S2, variable L1, variable L2, variable BS, variable del_r, variable usingLenses, variable r_dist, variable &SigmaQX, variable &SigmaQY, variable &fSubS)
...
//
Function xN_DetectorPixelResolution(string fileStr, string detStr)

	variable DDet
	//	String instr=fileStr[1,3]	//filestr is "[NGnSANSn] " or "[NGnSANSnn]" (11 characters total)

	string instr = StringFromList(2, fileStr, ".") // from "sansNNN.nxs.ng?" get the last three chars
...
//
Function xN_DetectorDeadtime(string fileStr, string detStr, [string dateAndTimeStr, variable dtime])
...
//
Function N_ConvertNexusDay2secs(string dateAndTime)
...
Function N_ConvertNexusDateTime2secs(string dateAndTime)
...
//
Function N_monStr2num(string monStr)
...
Function/S N_RunDigitString(variable num)
...
// was useful for the VAX files, but now there is no file prefix - the file
// names are all of the form 'sansNNNNN.nxs.ngNNN'
//
// so the prefix is always "sans"
//
// NCNR-specifc, does not really belong here - but it's a beta procedure used for the
// Combine Files Panel
//
Function/S N_GetPrefixStrFromFile(string item)
...
//
Function N_GetRunNumFromFile(string item)
...
//
Function/S N_FindFileFromRunNumber(variable num)
...
// .DAT
//
Function N_CheckIfRawData(string fname)
...
//
Function N_CheckIfDIVData(string fname)
...
Function N_isTransFile(string fname)
...
//
Function/S N_RemoveAllSpaces(string str)
...
//
Function/S N_FindValidFilename(string partialName)
...
// local function
//
Function/S N_ValidFileString(string partialName)
...
//
Function/S N_GetFileNameFromPathNoSemi(string fullPath)
...
//
Function/S N_GetFileNameFromPathKeepSemi(string fullPath)
...
//
Function/S N_GetPathStrFromfullName(string fullPath)
...
//
Function/S N_GetNameFromHeader(string fullName)
...
//
Function/S N_ParseRunNumberList(string list)
...
// local function
//
Function/S N_ExpandNumRanges(string list)
...
// local function
//
Function/S N_ListFromDash(string item)
...
////
//// -- to be used for the NGB 30m SANS, formerly located at NG3
////
////
...
////between calibrated wavelengths for a given attenuator
////
//// local function - to be used for the 10m SANS
////
//// Mar 2010 - abs() added to attStr to account for ICE reporting -0.0001 as an attenuator position, which truncates to "-0"
////
...
//
Function N_CalculateAttenuationFactor(string fname)
...
//
Function N_CalculateAttenuationError(string fname)
...
//
Function/S N_ReducedDataFileList(string ctrlName)
...
Function/S N_GetRawDataFileList()
	...
	for(ii = 0; ii < num; ii += 1)
		...
		if(stringmatch(item, "*.nxs*")) // added 2014 for HDF5 testing
			newlist += item + ";"
		endif
		//print "ii=",ii
	endfor
...
// -1 => previous file
// 1 => next file
Function/S N_GetPrevNextRawFile(string curfilename, variable prevnext)
