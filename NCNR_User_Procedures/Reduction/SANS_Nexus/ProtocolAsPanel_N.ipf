...
//
Function DoCatShort(string ctrlName) : ButtonControl
...
//
Function RecallProtocolButton(string ctrlName) : ButtonControl
...
//
Function DeleteProtocolButton(string ctrlName) : ButtonControl
...
//
Function ResetToSavedProtocol(string nameStr)
...
//
Function DoneProtocolButton(string ctrlName) : ButtonControl
...
//
Function SaveProtocolButton(string ctrlName) : ButtonControl
...
//
Function MakeProtocolFromPanel(WAVE/T w)
...
//
Function PickSAMButton(string ctrlName) : ButtonControl
...
//
Function PickBGDButton(string ctrlName) : ButtonControl
...
//
Function PickEMPButton(string ctrlName) : ButtonControl
...
//
Function PickDIVButton(string ctrlName) : ButtonControl
...
//
Function PickMASKButton(string ctrlName) : ButtonControl
...
//
Function ReduceOneButton(string ctrlName) : ButtonControl
...
//
Function SetABSParamsButton(string ctrlName) : ButtonControl
...
//
Function ShowCATButton(string ctrlName) : ButtonControl
...
// checked is the Desired state, not previous state
Function DrkCheckProc(string ctrlName, variable checked) : CheckBoxControl

	SVAR drkStr = root:myGlobals:Protocols:gDRK
	if(checked == 1)
		//print "Unchecked on call"
		//this just asks for the filename, doesn't open the file
		string msgStr   = "Select the DRK file"
		string fullPath = ""
		string fileStr  = ""
		variable refnum
...
Function ShowProtoHelp(string ctrlName) : ButtonControl
...
//
Function SetAverageParamsButtonProc(string ctrlName) : ButtonControl
...
Function CheckProtocolSyntax(string ctrlName) : ButtonControl
...
//
Function AreFilesThere(string type, string reqFiles)
...
//
Function AddFilesInList(string type, string list)
...
//
//
Function ExecuteProtocol(string protStr, string samStr)
	...
	do
		if((cmpstr(samStr, "ask") == 0) || (cmpstr(samStr, "") == 0)) //zero if strings are equal
			err = LoadRawSANSData("", "RAW") //will prompt for file
			if(err)
...
//
Function UpdateDisplayInformation(string type)
...
Function AskForAbsoluteParams_Quest()
	...
	if(err == 1)
		...
		DoAlert 0, "select the empty beam file"
		err = LoadRawSANSData("", "RAW") //reads the RAW data
		UpdateDisplayInformation("RAW") //display the new type of data that was loaded
		if(err)
			PathInfo/S catPathName
			Abort "reduction sequence aborted"
		...
		string filename = getFileNameFromFolder("RAW")
		string tempStr //02JUL13
		PathInfo/S catPathName
...
Function UserSelectBox_Continue(string ctrlName) : buttonControl

	DoWindow/K junkWindow //kill panel
End

Function SelectABS_XYBox(variable &x1, variable &x2, variable &y1, variable &y2)
...
Function UserSelectABS_Continue(string ctrlName) : buttonControl
...
//
//
Function ExportProtocol(string ctrlName) : ButtonControl
...
//
//
Function ImportProtocol(string ctrlName) : ButtonControl
...
Function/S GetSAMList()

	//	String match="SAMPLE"
	//	String list = getFileIntentList(match,0)

	string intent  = "SAMPLE"
	string purpose = "SCATTERING"
	string list    = getFileIntentPurposeList(intent, purpose, 0)

	//	Printf "SAM files = %s\r",list
	return (list)
End

Function/S GetBGDList()
...
//
// getFileIntentPurposeList(intent,purpose,method)
Function/S GetEMPList()
...
//
// if something fancier is needed, I could grep for SANS_DIV stored in the title block of the
// HDF file structure
Function/S GetDIVList()
	...
	newList = Get_NotRawDataFileList()
	//	newList = RemoveEXTFromList(newlist,"hst")		// remove the event files
	//	newList = RemoveEXTFromList(newlist,"ave")		// remove the ave files
	//	newList = RemoveEXTFromList(newlist,"abs")		// remove the abs files
	//	newList = RemoveEXTFromList(newlist,"phi")		// remove the phi files
	//	newList = RemoveEXTFromList(newlist,"pxp")		// remove the pxp files
	//	newList = RemoveEXTFromList(newlist,"DS_Store")		// remove the DS_Store file (OSX only)
...
Function/S GetMSKList()
	...
	//	newList = RemoveEXTFromList(newlist,"phi")		// remove the phi files
	//	newList = RemoveEXTFromList(newlist,"pxp")		// remove the pxp files
	//	newList = RemoveEXTFromList(newlist,"png")		// remove the png files
	//	newList = RemoveEXTFromList(newlist,"jpg")		// remove the jpg files
	//	newList = RemoveEXTFromList(newlist,"DS_Store")		// remove the DS_Store file (OSX only)

	// keep only MASK files in the list
	num = ItemsInList(newList)

	string matchStr = "*MASK*" // this is part of the title of a SANS _MASK_ file
	for(ii = 0; ii < num; ii += 1)
		item = StringFromList(ii, newList, ";")
		val  = stringmatch(item, matchStr)
		if(val) // true if the string did match
	...
	//
	////	String match="MASK"		// this is part of the title of a SANS MASK file
	//	String match="SANS_MASK"		// this is part of the title of a SANS MASK file
	//	for(ii=0;ii<num;ii+=1)
	//		item=StringFromList(ii, newList , ";")
	//		Grep/P=catPathName/Q/E=("(?i)\\b"+match+"\\b") item
	////		Grep/P=catPathName/Q/E=("(?i)"+match) item
...
Function Proto_LoadDIV(string protStr)
	...
	if(cmpstr("none", protStr) != 0) // if !0, then there's a file requested
		if(cmpstr("ask", protStr) == 0)
			//ask user for file
			//			 junkStr = PromptForPath("Select the detector sensitivity file")
			Prompt divFileName, "DIV File", popup, GetDIVList()
			...
			LoadRawSANSData(divFileName, "DIV")
		else
			//assume it's a path, and that the first (and only) item is the path:file
			//list processing is necessary to remove any final comma
...
//
Function Proto_LoadFile(string fileStr, string activeType, string msgStr)
	...
	do
		...
		if((cmpstr(fileStr, "none") != 0) && (cmpstr(fileStr, "ask") != 0))
			...
			if(!filesOK)
				...
				err = AddFilesInList(activeType, fileStr)
