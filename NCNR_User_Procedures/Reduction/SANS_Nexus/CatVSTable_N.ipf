...
//waves in the table
Function AppendNotRAWFiles(WAVE/T w)
...
Function GetHeaderInfoToWave(string fname, string sname)
...
//which alreay was found to exist. sname is the file;vers to be written out,
//avoiding the need to re-extract it from fname.
Function WriteCatToNotebook(string fname, string sname)
...
Function Write_ABSHeader_toNotebook(string fname, string sname)
...
//which alreay was found to exist. sname is the file;vers to be written out,
//avoiding the need to re-extract it from fname.
Function WriteCatVSToNotebook(string fname, string sname)
...
//   INTENT = Sample, Empty Cell, Blocked Beam, Open Beam, Standard
//
Function CatTableHook(string infoStr)
		...
		case "mouseup": 
			...
			strswitch(S_selection) // S_selection is now the contextual selection
				...
				case "Print Values":
					...
					fVerifyImportantUnits(FileNames[V_StartRow])
