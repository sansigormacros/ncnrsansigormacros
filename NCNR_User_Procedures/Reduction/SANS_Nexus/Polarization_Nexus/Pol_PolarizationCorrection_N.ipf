...
Function RestorePolCorPanel()
	...
	WAVE/Z/T w = root:Packages:NIST:Polarization:PolCor_popState
...
Function SavePolCorPanelButton(STRUCT WMButtonAction &ba) : ButtonControl
...
Function RestorePolCorPanelButton(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function PolCor_FileListBoxProc(STRUCT WMListboxAction &lba) : ListBoxControl
...
//
Function PolCorPanelHook(STRUCT WMWinHookStruct &s)
...
// val = 1 to disable
// val = 0 to show
Function ToggleSelControls(string str, variable val)
...
Function PolCorTabProc(STRUCT WMTabControlAction &tca) : TabControl
...
// 0 = SAM, 1 = EMP, 2 = BGD
Function ChangeDataTab(variable tab)
...
Function PolCorHelpParButtonProc(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function LoadRawPolarizedButton(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function LoadPolarizedData(string pType)

	string parsedRuns, condStr
	string listStr = ""
	string runList = ""
	...
	if(tabNum == 0 || tabNum == 1)
		...
		ControlInfo/W=PolCor_Panel $("popup_" + num2str(0) + "_1") //use the condition of the SAM tab
	endif
	condStr = S_Value
...
//
Function AddToPolMatrix(WAVE matA, WAVE matA_err, string pType, variable tMid)
	...
	string listStr, condStr, condNote, decayNote, cellStr, t0Str, t1Str, runStr
	string fname = ""
...
Function MakePCResultWaves(string type, string pType)
...
Function TagLoadedData(string type, string pType)
...
Proc DisplayTaggedData(type, pType)
	...
	root :myGlobals:gDataDisplayType=type + "_" + pType

	UpdateDisplayInformation(type + "_" + pType)
...
//
Function PolCorButton(STRUCT WMButtonAction &ba) : ButtonControl
	...
	switch(ba.eventCode)
		case 2: // mouse up
			...
			if(exists("root:Packages:NIST:" + type + "_UU:PolCorDone") == 0)
				...
				Print "Polarization correction already done, skipping for type = " + type
				return (0)
			endif
			...
			// clear out the linear_data waves from the top level -- these shouln't be used since they cause
			// confusion about what the corrected data is, and this format can't be used with VSANS
			KillWaves/Z linear_data_UU_pc, linear_data_DU_pc, linear_data_DD_pc, linear_data_UD_pc
			KillWaves/Z linear_data_error_UU_pc, linear_data_error_DU_pc, linear_data_error_DD_pc, linear_data_error_UD_pc
...
// - crude right now
//
Function Display4XSButton(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function ChangeDisplayedPolData(STRUCT WMButtonAction &ba) : ButtonControl
...
Function ShowPolMatrixButton(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function getTimeMidpoint(string listStr)
...
//
//
Function ReducePolCorDataButton(string ctrlName) : ButtonControl
...
//
Function ExecutePolarizedProtocol(string protStr, string pType)
...
//
Function RecallPolCorProtocolButton(string ctrlName) : ButtonControl
...
//
Function ResetToSavedPolProtocol(string nameStr)
...
Function ExportPolCorProtocolButton(string ctrlName) : ButtonControl

	ExportProtocol(ctrlName)
	return (0)
End

Function ImportPolCorProtocolButton(string ctrlName) : ButtonControl
...
//
Function SavePolCorProtocolButton(string ctrlName) : ButtonControl
...
//
Function MakePolProtocolFromPanel(WAVE/T w)
...
//
Function Display_4(string type, string scaling)
	...
	// since this is not properly updated at each step and I want to remove these waves
	...
	if(V_flag == 0)
		...
		Button button0, pos={sc * 130, 65 * sc}, size={sc * 50, 20 * sc}, title="Do It", proc=Change4xsButtonProc
		...
		RemoveImage/Z data //remove the old images (same names, different data folders)
		...
		RemoveImage/Z data // and remove once more, just for spite.
...
Function Change4xsButtonProc(STRUCT WMButtonAction &ba) : ButtonControl
...
//
Function ClearPolCorEntries(STRUCT WMButtonAction &ba) : ButtonControl
