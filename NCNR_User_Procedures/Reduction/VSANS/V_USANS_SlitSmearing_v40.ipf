...
//
Function V_USANS_CalcWeights(basestr, dQv)
	String basestr
	Variable dQv
	...
	W1mat = (p <= q ) && (q <= USANS_N-2) ? V_CalcW1(p,q)  : 0
	print "Calculating W2..."
	W2mat = (p+1 <= q ) && (q <= USANS_N-1) ?  V_CalcW2(p,q) : 0
	print "Calculating Remainders..."
	Rmat = (q == USANS_N-1) ? V_CalcR(p) : 0
...
//
Function V_USANS_RE_CalcWeights(baseStr,pt1,pt2)
	String baseStr
	Variable pt1,pt2
	...
	Variable tref = startMSTimer
	print "Calculating W1..."
	W1mat = (p <= q ) && (q <= matN-2) ? V_CalcW1(p,q)  : 0
	print "Calculating W2..."
	W2mat = (p+1 <= q ) && (q <= matN-1) ?  V_CalcW2(p,q) : 0
	print "Calculating Remainders..."
	Rmat = (q == matN-1) ? V_CalcR(p) : 0
//	print "Summing weights..."
...
//	return slope
	...
	Button button_OK,proc=V_USANS_Slope_ButtonProc
	Button button_SlopeCalc,pos={270,310},size={100,20},title="Calculate",font="Geneva"
	Button button_SlopeCalc,proc=V_USANS_Slope_ButtonProc
	...
	SetVariable setvar_slope, proc=V_USANS_ManualSlope
...
Function V_USANS_CalculateSlope(basestr, nend)
	String basestr
	Variable nend
...
//
Function V_USANS_ManualSlope(ctrlName,varNum,varStr,varName) : SetVariableControl
	String ctrlName
	Variable varNum	
	String varStr		
	String varName
...
Function V_USANS_Slope_ButtonProc(ctrlName) : ButtonControl
	String ctrlName
	...
	strswitch (ctrlName)
		...
		case "button_SlopeCalc":
			USANS_m = V_USANS_CalculateSlope(basestr, USANS_slope_numpnts)
			ControlUpdate/W=USANS_Slope setvar_Slope
			break
	endswitch
...
//TODO:
// -- does this work for different values of dQv?
// - if so, which index fo I need to use? i or j?
//
Function V_CalcW1(i,j)
	Variable i,j
...
//	NVAR dQv = USANS_dQv
	...
	retval = Integrate1D(V_Remainder,lower,upper)
