...
//
Function writeVCALC_to_file(string fileName, string labelStr, string intent, variable group_id)
...
// writes out "perfect" detector calibration constants for all 8 tube banks + back detector
Function V_WritePerfectSpatialCalib(string filename)
...
// simulated count rate per tube can be ≈ 10^8, so I need dt >> 10^-15 to completely cancel this out
// (partly due to fake I(q), fake count time in file...)

// writes out "perfect" dead time constants for all 8 tube banks + back detector
Function V_WritePerfectDeadTime(string filename)
...
//
Function V_DetPanelCountReport(string fname)
