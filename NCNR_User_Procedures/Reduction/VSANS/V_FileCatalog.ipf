...
Function V_BuildCatVeryShortTable()
	...
	variable indefinite  = 0
	variable useIgorDraw = 1
...
//
//
Function V_CatTableHook(string infoStr)
		...
		case "mouseup": 
			...
			strswitch(S_selection)
				...
				case "Print Values":
					...
					V_fVerifyImportantUnits(FileNames[V_StartRow])
...
Function V_AppendNotRAWFiles(WAVE/T w)
...
Function V_GetHeaderInfoToWave(string fname, string sname)
...
Function V_CatVSANSTable_SortFunction(string ctrlName) // added by [davidm]
...
Function V_fVerifyImportantUnits(string fname)
	...
	tmpStr = "FILE:  "+fname+"\r"
	Notebook HdrWin,font="Helvetica Neue"
...
//// loop over all detectors
	...
	for(ii = 0; ii < nn; ii += 1)
		...
		if(cmpstr(detStr,"B") == 0)
			str = V_IdentifyBackDetectorType(fname, 1)
			sprintf tmpStr,"Detector description = %s\r", str
			Notebook HdrWin,fstyle=1,textRGB=(0,0,0),text=tmpStr
		endif
...
///// end loop over all detectors
	...
	// collimation type as identified
	str = V_IdentifyCollimation(fname)
	sprintf tmpStr,"Collimation type = %s\r", str
	Notebook HdrWin,fstyle=1,textRGB=(0,0,0),text=tmpStr
...
//	// beam stop size, diameter in [mm]
	...
	val = V_getSampleAp2_distance(fname)
