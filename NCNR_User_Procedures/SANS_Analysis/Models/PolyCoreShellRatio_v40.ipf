...
Proc PlotPolyCoreShellRatio(num,qmin,qmax)
	...
	Variable/G root:g_pcr
	g_pcr := PolyCoreShellRatio(coef_pcr,ywave_pcr,xwave_pcr)
...
Proc PlotSmearedPolyCoreShellRatio(str)								
	String str
	...
	Duplicate/O $(str+"_q") smeared_pcr,smeared_qvals				//**** mod
	SetScale d,0,0,"1/cm",smeared_pcr							//**** mod					
		
	Variable/G gs_pcr=0
	gs_pcr := fSmearedPolyCoreShellRatio(smear_coef_pcr,smeared_pcr,smeared_qvals)	//this wrapper fills the STRUCT
...
//AAO version
Function PolyCoreShellRatio(cw,yw,xw) : FitFunc
	Wave cw,yw,xw

#if exists("PolyCoreShellRatioX")
	yw = PolyCoreShellRatioX(cw,xw)
#else
	yw = fPolyCoreShellRatio(cw,xw)
#endif
	return(0)
End
...
//c	EQNS: 32-37
//c
Function fPolyCoreShellRatio(w,x) : FitFunc
	Wave w;Variable x
...
Function SmearedPolyCoreShellRatio(s) :FitFunc
	Struct ResSmearAAOStruct &s

////the name of your unsmeared model is the first argument
	Smear_Model_20(PolyCoreShellRatio,s.coefW,s.xW,s.yW,s.resW)
...
//
Function fSmearedPolyCoreShellRatio(coefW,yW,xW)
	Wave coefW,yW,xW
	...
	Variable err
	err = SmearedPolyCoreShellRatio(fs)
